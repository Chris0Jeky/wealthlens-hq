...
- [ ] H1-11 (M1) Embed the corpus: `retrieval/dense.py` + batch embedding script through the `llm/client.py` seam (model per ADR 0003), writing pgvector rows + index — done when every chunk has an embedding and cosine query returns sane neighbours on 3 spot checks. [deps: H1-09] [BLOCKED: needs OPENAI_API_KEY + real spend (Chris) — see tasks/ACTION-REQUIRED.md. The DB write + pgvector cosine-query half is buildable/verifiable with synthetic vectors; the embed-generation half is gated on the key.]
