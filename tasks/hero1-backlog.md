...
- [ ] H1-20 (M3) Publish the /ask response JSON schema (answer | refusal | over-budget variants) + schema-validity deterministic check — done when `evals/checks/deterministic.py` validates live responses against it. [deps: H1-18, H1-19]
  - NOTE (seeded from #477 codex review): H1-19 surfaces `ResolvedCitations.unresolved_chunk_ids` but does NOT rewrite the model's answer text, so a pruned citation's `[chunk:<id>]` marker still sits inline in `ComposedAnswer.text`. The serving policy is H1-20's: when `unresolved_chunk_ids` is non-empty, either sanitize the inline markers out of the served text or refuse — do not return the raw body alongside the pruned citation list, or a fabricated marker leaks to the user.
