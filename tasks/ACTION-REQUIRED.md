...
7. - [x] **Merge the `make ci-quick` reliability fix (PR #350)** — **P2, done 2026-09-03: PR #350 merged earlier; repo settings applied and verified**
   ...
   - **DONE 2026-09-03 (agent, directly verified):** #595 merged (`8f4b50a`), then `PUT /branches/main/protection` with the seven contexts required (`strict: false`, no review rule, force-push and deletion off) and `allow_auto_merge` set true; measured back: `required_status_checks.contexts` = 7, `allow_auto_merge = true`, `allow_squash_merge = false`. The PR that carries this tick was itself merged with `gh pr merge --auto --merge` as the live end-to-end proof.
   - **Repo-setting actions (2, one-time)** — for Dependabot auto-merge to work end-to-end. Both measured OFF on 2026-09-03 morning and ON by the same afternoon; done **together**, in this order:
   ...
   - **~~3. Allow GitHub Actions to create and approve pull requests~~ — not needed today, do not enable.** This was listed because the workflow ran `gh pr review --approve`, which failed with "GitHub Actions is not permitted to approve pull requests". Correction (2026-09-03, from review on #590): that setting *is* the switch that lets `GITHUB_TOKEN` approve — the earlier claim that it "does not lift it for `GITHUB_TOKEN` itself" was wrong. The step was removed anyway because `main` requires no review, so an approval is dead weight and the setting would widen repo-wide Actions permissions for no benefit. **Conditional:** if a required-review rule is ever added to `main`, enable this setting and restore the approve step in the same change, or Dependabot PRs will sit blocked for manual review.
   - **Prerequisite for step 1 (agent, PR #595):** the four CI workflows were path-filtered on `pull_request`, so a required context could stay *Expected* forever and nothing would auto-merge. #595 makes every CI workflow run on every PR; the seven contexts to require are `lint-and-test`, `build`, `test`, `lint-type-test (3.11)`, `lint-type-test (3.12)`, `analyze (javascript-typescript)`, `analyze (python)`.
