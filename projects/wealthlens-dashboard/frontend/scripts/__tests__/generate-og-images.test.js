...
import { OG_METADATA, getChartsMissingOgMetadata, } from "../../src/constants/ogMetadata";
...
describe("OG Metadata", () => {
    ...
    it("all metadata entries have required fields", () => {
        for (const [slug, entry] of Object.entries(OG_METADATA)) {
            ...
            expect(entry.sourceAccessDate, `${slug} missing sourceAccessDate`).toMatch(/^\d{4}-\d{2}-\d{2}$/);
        }
    });
    it("metadata keys match VALID_CHART_NAMES exactly", () => {
        ...
        for (const key of metadataKeys) {
            expect(chartNames.has(key), `"${key}" in OG_METADATA but not in VALID_CHART_NAMES`).toBe(true);
        }
    });
});
describe("OG Generated Images", () => {
    ...
    describe("image files", () => {
        it.each(chartNames)("generates %s.png as valid PNG", (chartName) => {
            ...
            expect(header.equals(PNG_MAGIC_BYTES), `${chartName}.png has invalid PNG header`).toBe(true);
        });
        it("generates og-default.png as valid PNG", () => {
            ...
            const fileBuffer = readFileSync(filePath);
            const header = fileBuffer.subarray(0, 8);
            expect(header.equals(PNG_MAGIC_BYTES), "og-default.png has invalid PNG header").toBe(true);
        });
        it.each(chartNames)("%s.png is under size limit", (chartName) => {
            const filePath = join(OUTPUT_DIR, `${chartName}.png`);
            expect(existsSync(filePath), `${chartName}.png does not exist`).toBe(true);
            const fileBuffer = readFileSync(filePath);
            expect(fileBuffer.length, `${chartName}.png exceeds ${MAX_FILE_SIZE_BYTES / 1024}KB limit`).toBeLessThan(MAX_FILE_SIZE_BYTES);
        });
        it("og-default.png is under size limit", () => {
            const filePath = join(OUTPUT_DIR, "og-default.png");
            expect(existsSync(filePath), "og-default.png does not exist").toBe(true);
            const fileBuffer = readFileSync(filePath);
            expect(fileBuffer.length, `og-default.png exceeds ${MAX_FILE_SIZE_BYTES / 1024}KB limit`).toBeLessThan(MAX_FILE_SIZE_BYTES);
        });
        it("generates correct number of images", () => {
            let count = 0;
            for (const chartName of chartNames) {
                if (existsSync(join(OUTPUT_DIR, `${chartName}.png`)))
                    count++;
            }
            if (existsSync(join(OUTPUT_DIR, "og-default.png")))
                count++;
