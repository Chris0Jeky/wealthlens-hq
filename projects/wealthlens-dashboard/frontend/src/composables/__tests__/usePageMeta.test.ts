...
function getAllMeta(attr: "property" | "name", key: string): string[] {
  const els = document.head.querySelectorAll<HTMLMetaElement>(`meta[${attr}="${key}"]`)
...
describe("usePageMeta", () => {
  ...
  it("sets meta description", () => {
    wrapper = mount(createComponent({ description: "A test description" }))
  ...
  it("sets OpenGraph tags", () => {
    ...
    expect(getMeta("property", "og:url")).toBe("https://example.com/charts/wealth-shares")
    expect(getMeta("property", "og:image")).toBe("https://example.com/og/wealth-shares.png")
  ...
  it("sets Twitter Card tags", () => {
    ...
    expect(getMeta("name", "twitter:description")).toBe("Housing affordability data")
    expect(getMeta("name", "twitter:image")).toBe("https://example.com/og/housing.png")
  })
  ...
  it("sets image alt text on both og and twitter", () => {
    ...
    expect(getMeta("name", "twitter:image:alt")).toBe("Alt text for the image")
    expect(getMeta("property", "og:image:alt")).toBe("Alt text for the image")
  })
  ...
  it("defaults og:url to base URL when not provided", () => {
    ...
    expect(getMeta("property", "og:url")).toBe("https://chris0jeky.github.io/wealthlens-hq")
  })
