...
describe("SharePanel", () => {
  ...
  it("includes chart title in share URLs", async () => {
    ...
    expect(windowOpenSpy).toHaveBeenCalledWith(
      expect.stringContaining(encodeURIComponent("Who owns wealth in the UK? — WealthLens UK")),
      "_blank",
      "noopener,noreferrer",
    )
  })
  ...
  it("reverts copy button after 2 seconds", async () => {
    ...
    const resetBtn = wrapper.find('button[aria-label="Copy chart link"]')
  ...
  it("updates aria-label on copy link button after copy", async () => {
    ...
    expect(btn.attributes("aria-label")).toBe("Link copied to clipboard")
  })
  ...
  it("has section landmark with aria-labelledby", () => {
    ...
    expect(section.attributes("aria-labelledby")).toBe("share-panel-heading")
  })
