...
function isValidDatasetList(obj: unknown): obj is { datasets: string[] } {
  if (typeof obj !== "object" || obj === null) return false
  const o = obj as Record<string, unknown>
  return Array.isArray(o.datasets) && o.datasets.every((d: unknown) => typeof d === "string")
}

function isValidAllMetadata(obj: unknown): obj is { datasets: DatasetMetadata[] } {
  if (typeof obj !== "object" || obj === null) return false
  const o = obj as Record<string, unknown>
  return Array.isArray(o.datasets) && o.datasets.every((d: unknown) => isValidMetadata(d))
}
...
describe("Static data validation", () => {
  ...
  it("public/data directory exists and contains JSON", () => {
    expect(existsSync(DATA_DIR), `expected static data at ${DATA_DIR}`).toBe(true)
  ...
  describe("freshness.json (badge data)", () => {
    it("each entry has a valid, non-future last_updated and a source", () => {
      ...
      for (const [slug, value] of entries) {
        ...
        expect(typeof value, `freshness["${slug}"] should be an object`).toBe("object")
        ...
        expect(typeof entry.source, `freshness["${slug}"].source`).toBe("string")
