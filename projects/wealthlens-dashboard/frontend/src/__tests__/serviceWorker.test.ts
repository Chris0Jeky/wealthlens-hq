...
const serviceWorkerSource = readFileSync(resolve(__dirname, "../../public/sw.js"), "utf-8")

describe("service worker", () => {
  it("keeps API handling within the registered GitHub Pages scope", () => {
    expect(serviceWorkerSource).toContain('url.pathname.startsWith("/wealthlens-hq/api/")')
    expect(serviceWorkerSource).not.toContain('url.pathname.startsWith("/api/")')
  })
