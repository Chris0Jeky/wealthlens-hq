...
def write_chunks(chunks: list[Chunk], *, engine: Engine, replace: bool = True) -> int:
    ...
    Idempotent by default (replace=True): the existing rows for each
    (source_id, document_id) being ingested are deleted first, so re-running the
    ingest refreshes those documents in place instead of duplicating them.
    Embeddings for replaced chunks cascade away (FK ON DELETE CASCADE, migration
    0002) and are regenerated by the embed step (H1-11). The replace is scoped to
    the (source_id, document_id) pairs in THIS batch, so re-running a partial
    slice (e.g. tabular-only) leaves other documents intact; a document removed
    from the corpus entirely is therefore not auto-pruned, which is acceptable
    while the slice is frozen until v1. Returns the number of chunks written.
    """
    for chunk in chunks:
        validate_chunk_provenance(chunk)
    if not chunks:
        logger.warning("write_chunks: no chunks to write (nothing ingested)")
        return 0

    rows = [_chunk_to_row(chunk) for chunk in chunks]
    doc_keys = sorted({(chunk.source_id, chunk.document_id) for chunk in chunks})
...
def ingest_slice(*, engine: Engine | None = None, processed_dir: Path | None = None) -> int:
    ...
    logger.info("collected %d tabular chunk(s); document corpus pending H1-08 (PDF chunking)", len(chunks))
    return write_chunks(chunks, engine=engine)
...
def main() -> None:
    ...
    if written == 0:
        # An empty ingest is safe (no DELETE ran), but it almost always means the
        # gitignored pipeline outputs are missing — say so loudly rather than
        # reporting a hollow success.
        logger.warning(
            "ingest-slice wrote 0 chunks: the processed pipeline outputs are likely missing. "
            "Regenerate them (make pipelines) under projects/wealthlens-dashboard/data/processed and re-run."
        )
    else:
